#!/usr/bin/env python3
"""
UDP Video Viewer - Low Latency Version
Displays H.264 video streams from Android devices with minimal delay
"""
...
class VideoDecoder:
    """Decode H.264 using ffmpeg subprocess - LOW LATENCY"""
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                # Check for new frame immediately after each packet batch
                if self.decoder:
                ...
                # On timeout, still check for decoded frames
                if self.decoder:
                    frame = self.decoder.get_frame()
                    if frame:
                        self.has_video = True
...
class VideoViewer:
    ...
    def __init__(self, devices):
        self.root = tk.Tk()
        self.root.title("MirageTestKit - UDP Video Viewer (Low Latency)")
    ...
    def setup_ui(self):
        ...
        tk.Label(title_frame, text="MirageTestKit - UDP Video Viewer (Low Latency)",
    ...
    def update_display(self):
        """Update all displays - faster refresh rate"""
        total_packets = 0
        total_frames = 0
        ...
        for name, recv in self.receivers.items():
            ...
            if frame:
                display_img = frame.resize((380, 240), Image.Resampling.BILINEAR)  # Faster than LANCZOS
            else:
        ...
        self.root.after(33, self.update_display)  # ~30fps refresh
...
def main():
    print("=" * 60)
    print("MirageTestKit - UDP Video Viewer (Low Latency)")
    print("=" * 60)
    ...
    print("\nStarting viewer (Low Latency Mode)...")
