...
std::string MacroApiServer::dispatch(const std::string& json_line) {
    try {
        ...
        if (method == "normalize_coords") {
            return make_result(id, handle_normalize_coords(device_id,
                params.value("x", 0), params.value("y", 0),
                params.value("basis_w", 0), params.value("basis_h", 0)));
        }
        if (method == "resolve_coords") {
            return make_result(id, handle_resolve_coords(device_id,
                params.value("x_norm", 0.0), params.value("y_norm", 0.0),
                params.value("prefer_space", std::string("preview"))));
        }
...
std::string MacroApiServer::handle_normalize_coords(const std::string& device_id, int x, int y, int basis_w, int basis_h) {
    json r;
