...
// =============================================================================
// SharedFrame - Reference-counted frame buffer for zero-copy delivery
// =============================================================================
// All consumers (GUI, AI, Macro, OCR) share the same buffer via shared_ptr.
// Eliminates redundant memcpy operations across the frame processing pipeline.
// =============================================================================
struct SharedFrame {
    std::shared_ptr<const uint8_t[]> rgba;   // Reference-counted RGBA buffer
    int width = 0;
    int height = 0;
    uint64_t frame_id = 0;
    std::string device_id;
    ...
    size_t size_bytes() const { return static_cast<size_t>(width) * height * 4; }
};

// Frame delivery
struct FrameReadyEvent : Event {
    std::string device_id;
    const uint8_t* rgba_data = nullptr;      // Legacy: will be deprecated
    int width = 0;
    int height = 0;
    uint64_t frame_id = 0;
    // Optional: where this frame came from (TCP local port, USB=0, unknown=-1)
    // Used for GUI diagnostics to correlate texture updates with receiver routes.
    int source_port = 0;

    // New: shared ownership frame (preferred over rgba_data)
