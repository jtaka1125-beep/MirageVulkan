            ...
            for (auto& [id, dev] : devices_) {
                ...
                if (!packet.empty()) {
                    if (send_raw(*dev, packet.data(), packet.size())) {
                        ...
                        sent_any = true;
                        // FIX-D: register for ACK timeout tracking
                        {
                            mirage::protocol::PacketHeader hdr;
                            if (mirage::protocol::parse_header(packet.data(), packet.size(), hdr) &&
                                ...
                                pa.retry_count = 0;
                                std::lock_guard<std::mutex> qlock(dev->queue_mutex);
                                dev->pending_acks[hdr.seq] = std::move(pa);
                            }
                        }
                    }
                }
