...
public:
    ...
    uint32_t send_pinch(const std::string& usb_id,
                        ...
                        int duration_ms, int angle_deg100 = 0);
    uint32_t send_longpress(const std::string& usb_id, int x, int y, int duration_ms = 800);
...
private:
    ...
    struct DeviceHandle {
        ...
        // ISSUE-15: lifetime error counter
        std::atomic<uint64_t> recv_errors_lifetime{0};
        ...
        std::map<uint32_t, PendingAck> pending_acks;  // protected by queue_mutex
    ...
    void process_pending_acks_();  // FIX-D: timeout/retry check
    ...
    // ISSUE-7: CV to wake send_thread on new commands
