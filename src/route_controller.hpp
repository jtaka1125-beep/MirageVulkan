...
public:
    ...
    struct RouteDecision {
        ...
        int main_fps = 60;     // Target FPS for main device


        int sub_fps = 30;      // Target FPS for sub devices

        int encode_max_size = 0; // 0=no change; 1000=downscale, 2000=full
    ...
    using QualityCommandCallback = std::function<void(const std::string& device_id, int encode_max_size)>;
    ...
    void setQualityCommandCallback(QualityCommandCallback cb) { quality_callback_ = cb; }
    ...
    void resetDeviceFps(const std::string& device_id);  // Force FPS re-send on next applyDecision


    bool isRegistered(const std::string& device_id) const { return devices_.count(device_id) > 0; }
...
private:
    ...
    static constexpr int CONGESTION_THRESHOLD = 4;   // 4 ticks before USB->WiFi offload


    static constexpr int FAILURE_THRESHOLD = 2;      // 2 ticks (~2s) before declaring failure


    static constexpr int RECOVERY_THRESHOLD = 5;     // 5 ticks before switching back to USB
    ...
    static constexpr int SUB_FPS_LOW   = 15;
    ...
    QualityCommandCallback quality_callback_;
    ...
    static constexpr float EWMA_ALPHA = 0.20f;  // smoother route decisions
