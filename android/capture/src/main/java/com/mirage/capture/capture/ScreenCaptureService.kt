...
class ScreenCaptureService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        // Check if device is X1 (Npad) - requires tiled encoding for full resolution
        val isX1 = android.os.Build.MODEL?.lowercase()?.contains("npad") == true
        val metrics = resources.displayMetrics
        val screenH = maxOf(metrics.widthPixels, metrics.heightPixels)
        val screenW = minOf(metrics.widthPixels, metrics.heightPixels)
