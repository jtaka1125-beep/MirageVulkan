...
class TileRepeater(
        ...
        try {
            ...
            while (running) {
                ...
                if (sleepNs > 1_000_000L) {
                    Thread.sleep((sleepNs / 1_000_000L).coerceAtMost(5L))
                    continue
                }
                // If far behind, resync
                if (-sleepNs > intervalNs * 5) {
                    nextNs = nowNs
                }
                nextNs += intervalNs

                // Always call updateTexImage (like SurfaceRepeater)
                try {
                ...
                for (oe in eglOuts) {
                    ...
                    // So we invert Y: tileY=0 should get top of image (high V values)
                    val u0 = oe.out.tileX.toFloat() / oe.out.tilesX.toFloat()
                    val u1 = (oe.out.tileX + 1).toFloat() / oe.out.tilesX.toFloat()
                    val v0 = 1.0f - (oe.out.tileY + 1).toFloat() / oe.out.tilesY.toFloat()
                    val v1 = 1.0f - oe.out.tileY.toFloat() / oe.out.tilesY.toFloat()
    ...
    private fun createSurfaceTexture() {
        surfaceTexture = SurfaceTexture(texId).apply {
            ...
            // Use main thread Handler for OnFrameAvailable callback
            // (GL thread has no Looper, so callback wouldn't work without a Handler)
        ...
        Log.i(TAG, "SurfaceTexture created: ${targetW}x${targetH}, texId=$texId, surface=$inputSurface")
    }
