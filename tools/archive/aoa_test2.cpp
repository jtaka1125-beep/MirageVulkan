// AOA Switch Test v2 - longer wait, multiple rescans
#include <cstdio>
#include <cstring>
#include <chrono>
#include <thread>
...
void scan_aoa(libusb_context* ctx) {
    libusb_device** devs;
    ssize_t cnt = libusb_get_device_list(ctx, &devs);
    printf("  Scan: %d USB devices total\n", (int)cnt);
    int aoa = 0;
    for (ssize_t i = 0; i < cnt; i++) {
        ...
        if (desc.idVendor == 0x18D1 && desc.idProduct >= 0x2D00 && desc.idProduct <= 0x2D05) {
            printf("  -> AOA device: VID=%04x PID=%04x\n", desc.idVendor, desc.idProduct);
            aoa++;
        }
        // Also check for MediaTek still present
        if (desc.idVendor == 0x0E8D) {
            printf("  -> MediaTek device still present: VID=%04x PID=%04x\n", desc.idVendor, desc.idProduct);
        }
    }
    printf("  AOA: %d, MediaTek remaining: check above\n", aoa);
...
int main() {
    ...
    for (ssize_t i = 0; i < cnt; i++) {
        ...
        printf("[%zd] MediaTek VID=%04x PID=%04x\n", i, desc.idVendor, desc.idProduct);
        libusb_device_handle* h = nullptr;
        int r = libusb_open(devs[i], &h);
        if (r != 0) { printf("  OPEN FAILED: %s\n", libusb_error_name(r)); continue; }

        uint8_t ver[2] = {0};
        r = libusb_control_transfer(h, LIBUSB_ENDPOINT_IN | LIBUSB_REQUEST_TYPE_VENDOR,
            AOA_GET_PROTOCOL, 0, 0, ver, 2, 1000);
        if (r < 0) { printf("  AOA query failed: %s\n", libusb_error_name(r)); libusb_close(h); continue; }
        printf("  AOA version: %d\n", ver[0] | (ver[1]<<8));
        ...
        r = libusb_control_transfer(h, LIBUSB_ENDPOINT_OUT | LIBUSB_REQUEST_TYPE_VENDOR,
    ...
    if (switched == 0) { printf("No devices switched.\n"); libusb_exit(ctx); return 0; }

    printf("\nSwitched %d device(s). Scanning every 2s for 20s...\n", switched);
